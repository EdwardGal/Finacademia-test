...
const mainNews = document.querySelector('.main-news__block');
const newsPartners = document.querySelector('.partners-news__block');

if (mainNews) {
	const mainNewsShow = mainNews.dataset.show;
	const itemsMore = document.querySelector('.main-news__button');
	let startHeight = 0;
	let allHeight = 0;
	...
	itemsMore.addEventListener("click", function (e) {
		...
		if (!itemsMore.classList.contains('_active')) {
			mainNews.style.height = `${startHeight}px`;
		} else {
			mainNews.style.height = `${allHeight}px`;
			mainNews.addEventListener("transitionend", setHeight);
		}
		e.preventDefault();
	});
}
if (newsPartners) {
	const newsPartnersShow = newsPartners.dataset.show;
	const itemsMore = document.querySelector('.partners-news__button');
	let startHeight = 0;
	let allHeight = 0;
	...
	function setHeight(update = false) {
		const newsPartnersItems = document.querySelectorAll('.partners-news__item');
		const windowWidth = getClientWidth();
		if (update) {
			startHeight = 0;
			allHeight = 0;
		}
		newsPartnersItems.forEach((item, index) => {
			allHeight += item.offsetHeight;
			if (index < newsPartnersShow) {
				startHeight += item.offsetHeight;
			}
		});
		let currentHeight = itemsMore.classList.contains('_active') ? allHeight : startHeight;
		newsPartners.style.height = `${currentHeight}px`;
	...
	itemsMore.addEventListener("click", function (e) {
		...
		if (!itemsMore.classList.contains('_active')) {
			newsPartners.style.height = `${startHeight}px`;
		} else {
			newsPartners.style.height = `${allHeight}px`;
			newsPartners.addEventListener("transitionend", setHeight);
		}
		e.preventDefault();
	});
}
...
let rus = {
	mainTitle: 'Финакадемия',
	mainDesc: 'Консалтинговая компания, специализирующаяся на предоставлении организациям и частным лицам финансовых услуг в качестве агрегатора',
	mainText: ' Генератор оптимальных финансовых решений',
	mainButton: 'Наша команда',
	menuOne: 'О компании',
	menuTwo: 'Физическим лицам',
	menuThree: 'Предприятиям',
	menuFour: 'Инвестиции',
	menuFive: 'Блог',
};

let eng = {
	mainTitle: 'Finacademia',
	mainDesc: 'A consulting company specializing in providing organizations and private individuals of financial services as an aggregator',
	mainText: 'Generator of optimal financial decisions',
	mainButton: 'Our team',
	menuOne: 'About company',
	menuTwo: 'For individuals',
	menuThree: 'For enterprises',
	menuFour: 'Investments',
	menuFive: 'Blog',
};
...
function changeLagnuage() {
	...
	document.querySelectorAll('[data-lang]').forEach(el => {
		el.innerHTML = language[el.getAttribute('data-lang')];
	})
}
